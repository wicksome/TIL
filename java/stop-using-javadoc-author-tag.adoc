...
번역에 서툴러 매끄럽지 못하거나 어색한 부분에 있을 수 있으며 이러한 부분에 대해 피드백 주시면 감사하겠습니다.
원문 링크는 다음과 같습니다.

https://www.vojtechruzicka.com/stop-using-javadoc-author-tag/["Stop using Javadoc @author tag"] (2019-03-07) by https://www.vojtechruzicka.com/about/[Vojtech Ruzicka]
****
...
기본적으로 Javadoc을 생성하는데 있어서 디폴트 옵션이 아니기 때문에 `@author` 태그의 내용은 포함되지 않는다. 문서에 author 정보를 포함하기 위해서는
...
개발자는 코드 변경시 주석도 함께 업데이트하는 것을 잊거나 무시할 때가 많으며, 리팩토링 후에는 원작자가 개발했을 때와 전혀 다른 코드게 되기 마련이다.
...
이것은 `@author` Javadoc의 모든 단점을 포함하고 있다. Javadoc과 달리, IDE를 통해 {code-reference-info}[Javadoc 팝업]을 호출해도 보여지지 않을 뿐더러 자동 완성도 활용할 수 없다.
...
거대하고 오래된 코드베이스로 작업하는 동안에는 누가 기여자인지 아는 것보단 특정 파일 또는 모듈과 관련해서 누구와 연락해야 하는지는 아는 것이 훨씬 더 중요하다.
그러니 태그를 최신 상태로 유지해라. 그러나 최신 상태로 유지해야 할 파일이 많다면 개별 파일이 아닌 전체 모듈의 {javadoc-package}[author information on package level]를 포함하는 것이 나을 수 있다.
...
`@author` 는 코드에 대한 오너쉽을 가지기 위한 정보가 아니다. 내 코드라고 명시할 이유가 없다. 우리 팀 내 모든 코드에 대해서 오너쉽을 가져야 하며,
...
(이렇게 기능이 제공된다는 것이 충돌은 항상 일어나고 해결해나가야 한다는 의미가 아닐까.)
