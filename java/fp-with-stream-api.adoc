...
.reduce vs collect
====
reduce는 누산기^accumulator^를 사용해 값을 모으며, 변경 불가능한(immutable) 결과들로 동작하도록 설계되어 있다.
그에 반해, collect는 변경 가능한(mutable) 결과 객체를 사용하여 동작한다.

* https://stackoverflow.com/questions/22577197/java-8-streams-collect-vs-reduce[Java 8 Streams - collect vs reduce]
* https://12bme.tistory.com/468[자바8 스트림으로 데이터 수집 (Collector)]
...
[IMPORTANT]
._Stream.forEach()_ vs _Collection.forEach()_
====
두 코드의 결과는 동일하다. 하지만... TODO
[source, java]
----
// iterable
