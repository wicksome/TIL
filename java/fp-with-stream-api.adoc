    ...
    try {
        return Stream.of(id)
            ...
            .map(user -> {
                ...
                final CompletableFuture<Accounts> fAccounts = accountConnector.getAccounts(uuid);
                final CompletableFuture<Cards> fCards = cardConnector.getCards(uuid);
                final CompletableFuture<Point> fPoint = pointConnector.getPoint(uuid);
