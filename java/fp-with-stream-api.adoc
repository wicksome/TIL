...
_CompletableFuture_ 작업을 진행하던 중 에러가 발생하게 되면 _exceptionally_ 메서드를 통해 처리할 수 있다.
...
private List<User> restTemplate(List<String> ids) {
    List<ResponseEntity<User>> res = new ArrayList<>(ids.size());

    ids.forEach(id -> res.add(restTemplate.getForEntity(
        SLOW_HOST + "/api/Users/" + id, User.class)));
