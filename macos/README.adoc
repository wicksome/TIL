...
* [.line-through]#https://www.macbartender.com/[Bartender]# - 메뉴바 관리 → https://matthewpalmer.net/vanilla/[vanilla] → https://github.com/dwarvesf/hidden[hidden]
* https://github.com/Clipy/Clipy[Clipy] - 클립보드
** [.line-through]#https://www.raycast.com/#
** [.line-through]#https://www.alfredapp.com/#
* https://iterm2.com/index.html[iTerm2] - 터미널 `brew install --cask iterm2`
** ghostty - 터미널 `brew install --cask ghostty`
* 1Password
...
https://github.com/wicksome/dotfiles/tree/master/.hammerspoon
...
[alias]
    ...
    lgn = log --graph --abbrev-commit --decorate --date=format:'%y/%m/%d %H:%M' --format=format:'%C(bold blue)%h%C(reset) %C(dim blue)%ad%C(reset) %C(white)%s%C(reset) %C(dim white)- %an%C(reset)%C(bold yellow)%d%C(reset)'
----
. 개인/업무 디렉토리 분리
+
[source, bash]
